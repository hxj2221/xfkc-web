  ...
  <div class="adm-challenge">
    ...
    <div class="center-title">
      <div class="select-item">
        ...
        >正在进行</span>
        <span
          :class="['item-retrieval','border-blue',retrieval === 3 ? 'background-blue-active' : '' ]"
          @click="changeRet(3)"
        >已结束</span>
      </div>
...
export default {
  ...
  methods: {
    ...
    Pchallenges() {
      this.$router.push({ path: '/Pchallenge/Pchallenge' })
    },

    gotolist(item) {
      this.$router.push({ path: '/challengeList/challengeList?id=' + item.id })
    },
    handleSizeChange() {
      this.getFindMyChallenge()
    },
    ...
    toChange(item) {
      this.$router.push({ path: '/Pchallenge/Pchallenge?id=' + item.id })
    },
    changeRet(val) {
      this.retrieval = val
      const params = {
