...
<script>
import { getCourseFeatureDetail, getCourseFeatureQuery, giveCollect, giveLike, giveScore } from '@/api/featureCourse/index'
export default {
  name: '',
  props: {
    id: {
  ...
  methods: {
    getDetail(id) {
      getCourseFeatureDetail({ id: id }).then(res => {
        if (res) {
          this.sourceData = res
    ...
    getList() {
      ...
      getCourseFeatureQuery(params).then(res => {
