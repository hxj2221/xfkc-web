...
<script>
import { getSourceCategory } from '@/api/featureCourse/index'
export default {
  name: '',
  components: {
  },
  ...
  methods: {
    ...
    changeCategory(type, val) {
      if (type === 'one') {
        ...
        this.twoCategoryId = val
        this.$emit('changeShowType', 'list', this.twoCategory)
      }
    },
    changeTab(info) {
      this.oneCategory = info.featureCategoryId1
      getSourceCategory({ id: info.featureCategoryId1 }).then(res => {
        this.twoCategory = info.featureCategoryId2
      })
    }
  }
}
