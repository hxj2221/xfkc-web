      ...
      <div v-for="(item) in pointList1" :key="item.id" class="content-item" @click="changeDetail(item.id)">
        <div class="item-img">
          <el-image
            ...
            :src="$comm.url(item.badgeUrl)"
            fit="cover"
          />
          <!-- <img :src="baseApi + '/file/' +item.badgeUrl" alt> -->
        </div>
        <div class="item-text">
          <div class="item-text-title">{{ item.name }}</div>
          <div class="item-text-time">
            <el-image :src="require('../../assets/achievement/sun.png')" style="height:20px;width:20px" alt="" />
            已有{{ item.numberOfPeople }}人完成
          </div>
        </div>
        <el-image v-if="item.isOwned" class="finish-img" :src="require('../../assets/challenge/finish.png')" alt />
      </div>
    </div>
    <div class="paging">
      <div class="myTwo">
