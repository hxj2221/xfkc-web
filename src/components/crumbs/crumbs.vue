  ...
  computed: {
    navtitle() {
      return function(meta) {
        ...
        if (meta.title === '领衔校') {
          const schoolmap = ['基地校', '领衔校', '重点校', '一般项目校']
          const index = isNaN(+this.$route.query.id) ? 1 : this.$route.query.id > 4 ? 3 : +this.$route.query.id - 1
          console.log('index', isNaN(+this.$route.query.id), 'index2', index)
